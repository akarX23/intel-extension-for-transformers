...
for name in list_vars.keys():
    ...
    m = fetch_module(model, name)
    # if isinstance(m, auto_gptq.nn_modules.qlinear.qlinear_cuda_old.QuantLinear):
    #     if m.bits in [2,4,8]:
    #         zeros = torch.bitwise_right_shift(torch.unsqueeze(m.qzeros, 2).expand(-1, -1, 32 // m.bits), m.wf.unsqueeze(0)).to(torch.int16 if m.bits == 8 else torch.int8)
    #         torch.bitwise_and(zeros, (2 ** m.bits) - 1, out=zeros)
    ...
    #         weight = torch.bitwise_right_shift(torch.unsqueeze(m.qweight, 1).expand(-1, 32 // m.bits, -1), m.wf.unsqueeze(-1)).to(torch.int16 if m.bits == 8 else torch.int8)
    #         torch.bitwise_and(weight,(2 ** m.bits) - 1, out=weight)
    #         weight = weight.reshape(-1, m.group_size, weight.shape[2])

    #     weight = (scales * (weight - zeros))
    #     weight = weight.reshape(weight.shape[0] * weight.shape[1], weight.shape[2])
    #     print(f"gptq {name}")
    #     weight.numpy().tofile(f)
    # else:
    print(f"float {name}")
